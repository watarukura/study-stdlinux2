...
static struct HTTPRequest *reqd_request(FILE *in);
static void reqd_request_line(struct HTTPRequest *req, FILE *in);
...
static struct HTTPRequest *reqd_request(FILE *in) {
  ...
  reqd_request_line(req, in);
  req->header = NULL;
  while (h = reqd_header_field(in)) {
    h->next = req->header;
    req->header = h;
  }
  ...
  if (req->length != 0) {
    ...
    if (freqd(req->body, req->length, 1, in) < 1)
...
static void reqd_request_line(struct HTTPRequest *req, FILE *in) {
  ...
  if (!fgets(buf, LINE_BUF_SIZE, in))
    log_exit("no request line") p = strchr(buf, ' ');
  ...
  *p++ = '\0';
  ...
  if (strncasecmp(p, "HTTP/1.", strlen("HTTP/1.")) != 0)
    log_exit("parse erroro on requst line (3): %s", buf);
  p += strlen("HTTP/1.");
...
static long contnt_length(struct HTTPRequest *req) {
  char *val;
  long len;
...
static char *lookup_header_field_value(struct HTTPRequest *req, char *name) {
  ...
  for (h = req->header; h; h = h->next) {
    if (strcasecmp(h->name, name) == 0) return h->value;
  }
  return NULL;
}
